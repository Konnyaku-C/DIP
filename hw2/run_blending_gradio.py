...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = laplacian_kernel.view(1, 1, 3, 3).repeat(1, 3, 1, 1)
    laplacian_kernel = laplacian_kernel.to(foreground_img.device)

    foreground_laplacian=torch.nn.functional.conv2d(foreground_img,laplacian_kernel,padding=1)
    blended_laplacian=torch.nn.functional.conv2d(blended_img,laplacian_kernel,padding=1)
  ...
  #  masked_blended_laplacian=blended_laplacian*background_mask
    ...
    loss = loss + torch.sum((foreground_laplacian[foreground_mask_index] - blended_laplacian[background_mask_index])**2)
  #  loss=torch.nn.MSELoss(masked_foreground_laplacian,masked_blended_laplacian)
