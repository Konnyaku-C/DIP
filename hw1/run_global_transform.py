...
# Function to apply transformations based on user inps
def apply_transform(image, scale, rotation, translation_x, translation_y, flip_horizontal):
    ...
    M_transform=cv2.getRotationMatrix2D(center,rotation,scale)#输出一个2*3矩阵
    M_transform[:,2]=M_transform[:,2]+[translation_x,translation_y]#使用齐次坐标 加上平移矩阵
    image=cv2.warpAffine(image,M_transform,(w,h))
